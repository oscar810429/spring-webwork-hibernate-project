...
<div id="msgTabs">
<ul>
	<li><a class="selected" href="${urls.getURL("/profile/messages/inbox")}">接收到的留言</a></li>
...
<#list result.data as message>
	<li id="msg_${message.id}">
		...
		<span class="opts">
			<a href="${urls.getURL("/profile/messages/compose")}?to=${message.sender.id}"">回复</a>
...
<#if (result.totalPage > 1)>
	<div class="pages">
		<@pn.pager result="result" url="${urls.getURL('/profile/messages/inbox')}?page=$PAGE" />
	</div>
</#if>
</div>
</div>