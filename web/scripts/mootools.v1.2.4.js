		...
		return {
			...
			options: {
				...
				handleResponse: function(response){
					...
					if (this.options.wrapWithUi) response = StickyWin.ui(this, response, this.options.uiOptions);
...
StickyWin.UI = new Class({
    Implements: [Options, Class.ToElement],
    options: {
        width: 300,
        cornerHandle: false,
        cssClass: "",
        buttons: [],
        cssClassName: "x-dialog"
    },
    ...
    build: function() {
        ...
        var a = new Element("div", {
            "class": b.cssClassName
        });
        if (b.width) {
            a.setStyle("width", b.width)
        }
        this.element = a;
        this.element.store("StickyWinUI", this);
        if (b.cssClass) {
            a.addClass(b.cssClass)
        }
        this.body = new Element("div", {
            "class": "body"
        }).inject(a);
        this.shadow = new leopard.Shadow({
            mode: "frame",
            offset: 5
        });
        this.win.addEvents({
            display: this.shadow.show.bind(this.shadow, this.element),
            close: this.shadow.hide.bind(this.shadow)
        });
        return this
    },
    setContent: function(a) {
        if (document.id(a)) {
            this.body.empty().adopt(a)
        } else {
            this.body.set("html", a)
        }
        return this
    }
});
...
StickyWin = Class.refactor(StickyWin, {
    initialize: function(a) {
        this.previous(a);
        StickyWin.instances.set(this.id, this)
    }
});
StickyWin.instances = new Hash();

var leopard = {
		version: "1.0.1"
};

leopard.API = new Class({
    Extends: Request,
    options: {
    ...
    success: function(b) {
        var a = JSON.decode(b, true);
        if (a.stat == "ok") {
            this.onSuccess(a, b)
        } else {
            this.fireEvent("complete").fireEvent("error", a)
        }
    },
    call: function() {
        ...
        a.data = $extend({
            api_key: this.options.apiKey,self:this.options.self
        },
        a.data || {});
        if (this.options.defaultParams) {
            a.data = $extend(a.data, this.options.defaultParams)
        }
        return this.send({
...
function $api() {
    ...
    return new leopard.API($extend(a.options || {},
    {
        defaultParams: {
            method: a.method
        }
...
/*var urls = {
	    SCHEME: "http",
	    APP_DOMAIN: window.app_domain,
	    MEDIA_ROOT: window.media_root,
	    MEDIA_FILES: /(\.gif)|(\.jpg)|(\.png)|(\.css)|(\.js)$/i,
	    _: function(a) {
	        if (a.indexOf("/") === 0) {
	            return urls.SCHEME + "://www." + urls.APP_DOMAIN + a
	        }
	        return a
	    },
	    _u: function() {
	        ...
	        if (a.length == 1) {
	            if (typeof a[0] == "string") {
	                return urls._u(window.current_user, a[0])
	            } else {
	                return urls._u(a[0], "/")
	            }
	        }
	        if (a.length === 2) {
	            if (a[1].indexOf("/") === 0) {
	                ...
	                return urls.SCHEME + "://" + b + "." + urls.APP_DOMAIN + a[1]
	            }
	            return a[1]
	        }
	    },
	    ...
	    _p: function(b, a, c) {
	        ...
	        return "http://photo." + urls.APP_DOMAIN + "/" + b.bucket + "/" + b.key + "/" + a + "/"
	    },
	    _pv: function(c, e) {
	        var d = c.id.split("-")[1];
	        var b = "/photos/" + c.ownername;
	        if (e) {
	            if (e.album) {
	                var a = e.album.split("-")[1];
	                b += "/albums/" + a
	            }
	        }
	        b += "/" + d + "/";
	    ...
	    album: function(b) {
	        var a = b.id.split("-")[1];
	        return this._("/photos/" + b.ownername + "/albums/" + a + "/")
	    },
	    album_cover: function(a, b) {
	        if (!a.cover) {
	            return this._m("/img/default_set_photo.jpg")
	        }
	        return this._p(a.cover, b)
	    }
	};
	function $url() {
	    var a = $A(arguments);
	    if (a.length > 1) {
	        return (a[0].key) ? urls._p.apply(urls, a) : urls._u.apply(urls, a)
	    }
	    if (a.length === 0) {
	        return urls._u()
	    }
	    return (a[0].test(urls.MEDIA_FILES)) ? urls._m(a[0]) : urls._(a[0])
	}*/
/**
 * Utility functions
 */
var urls = {
	SCHEME: 'http',
	APP_DOMAIN: window.app_domain || 'painiu.com',
	MEDIA_ROOT: window.media_root || 'http://www.painiu.com',
	MEDIA_FILES: /(\.gif)|(\.jpg)|(\.png)|(\.css)|(\.js)$/i,
	PHOTO_SUFFIX: { 'square': '_s', 'thumb': '_t', 'small': '_m', 'medium': '', 'large': '_o' ,'organize':'_$secret'},
	
	_: function(path) {
		if (path.indexOf('/') === 0) {
			return urls.SCHEME + '://www.' + urls.APP_DOMAIN + path;
		}
		return path;
	},
	_u: function() {
		...
		if (args.length === 2) {
			if (args[1].indexOf('/') === 0) {
				...
				return urls.SCHEME + '://' + username + '.' + urls.APP_DOMAIN + args[1];
			}
			return args[1];
		}
	},
	_m: function(path) {
		if (path.indexOf('/') === 0) {
			return urls.MEDIA_ROOT + path;
		}
		return path;
	},
	_p: function(photo, version, context) {
		if (version == 'view') { return urls._pv(photo, context); }
		if(version=="organize"){
			return 'http://photo' + photo.host + '.' + urls.APP_DOMAIN + '/' + photo.dir + '/' + photo.filename +'_'+ photo.secret + '.jpg';
		}else{
			return 'http://photo' + photo.host + '.' + urls.APP_DOMAIN + '/' + photo.dir + '/' + photo.filename + urls.PHOTO_SUFFIX[version] + '.jpg';
		}	
	},
	
	_pv: function(photo, context) {
		var url = urls._('/photos/view?id='+photo.id);
		if (context) { url += '&' + Object.toQueryString(context); }
		return url;
	}
};
...
function $url() {
	var args = $A(arguments);
	if (args.length > 1) { return (args[0].filename)? urls._p.apply(urls, args) : urls._u.apply(urls, args); }
	if (args.length === 0) { return urls._u(); }
	return (args[0].test(urls.MEDIA_FILES)) ? urls._m(args[0]) : urls._(args[0]);
}
	...
	leopard.Shadow = new Class({
	    ...
	    initAdjusts: function() {
	        ...
	        var b = Math.floor(this.options.offset / 2);
	    ...
	    show: function(a) {
	        a = document.id(a);
	        if (!this.el) {
	            this.el = leopard.Shadow.Pool.pull();
	            this.el.injectBefore(a);
	        }
	        this.el.setStyle("z-index", this.zIndex || a.getStyle("z-index") - 1);
	        if (Browser.Engine.trident4) {
	            this.el.style.filter = "progid:DXImageTransform.Microsoft.alpha(opacity=80) progid:DXImageTransform.Microsoft.Blur(pixelradius=" + (this.offset) + ")"
	        }
	        var b = a.getCoordinates(a.getParent());
	        this.realign(b.left, b.top, b.width, b.height);
	        this.el.show();
	    },
	    isVisible: function() {
	        return this.el ? true: false;
	    },
	    realign: function(b, p, n, f) {
	        if (!this.el) {
	            return;
	        }
	        var k = this.adjusts,
	        ...
	        var o = (n + k.w),
	        e = (f + k.h);
	        var g = Math.max(0, o) + "px",
	        m = Math.max(0, e) + "px";
	        if (q.width != g || q.height != m) {
	            q.width = g;
	            q.height = m;
	            if (!Browser.Engine.trident4) {
	                var j = i.childNodes;
	                var c = Math.max(0, (o - 12)) + "px";
	                j[0].childNodes[1].style.width = c;
	                j[1].childNodes[1].style.width = c;
	                j[2].childNodes[1].style.width = c;
	                j[1].style.height = Math.max(0, (e - 12)) + "px"
	            }
	        }
	    },
	    hide: function() {
	        if (this.el) {
	            this.el.dispose();
	            leopard.Shadow.Pool.push(this.el);
	            delete this.el;
	        }
	    },
	    setZIndex: function(a) {
	        ...
	        if (isNaN(a)) {
	            this.el.setStyle("z-index", a.getStyle("z-index") - 1);
	        } else {
	            this.el.setStyle("z-index", a);
	        }
	    }
	});
	leopard.Shadow.Pool = function() {
	    var b = [];
	    var a = '<div class="xst"><div class="xstl"></div><div class="xstc"></div><div class="xstr"></div></div><div class="xsc"><div class="xsml"></div><div class="xsmc"></div><div class="xsmr"></div></div><div class="xsb"><div class="xsbl"></div><div class="xsbc"></div><div class="xsbr"></div></div>';
	    return {
	        pull: function() {
	            var c = b.shift();
	            if (!c) {
	                c = new Element("div");
	                if (Browser.Engine.trident4) {
	                    c.addClass("x-ie-shadow")
	                } else {
	                    c.addClass("x-shadow").set("html", a)
	                }
     ...
     (function() {
         leopard.assets = {
	        images: {
	            working: $url("/images/working.gif"),
	            loading: $url("/images/ajax-loading.gif"),
	            error: $url("/images/spinner-error.png"),
	            alert: $url("/images/spinner-alert.png"),
	            success: $url("/images/spinner-ok.gif")
	            //shadow: $url("/images/base/shadow.png"),
	            //shadow_lr: $url("/images/base/shadow-lr.png"),
	            //shadow_c: $url("/images/base/shadow-c.png"),
	            //bg_pane_corner_png: $url("/img/menu/bg-pane-corner.png"),
	            //bg_pane_corner_gif: $url("/img/menu/bg-pane-corner.gif"),
	            //bg_pane_tb: $url("/img/menu/bg-pane-tb.gif"),
	            //bg_pane_cl: $url("/img/menu/bg-pane-cl.gif"),
	            //bg_pane_cr: $url("/img/menu/bg-pane-cr.gif")
	        },
	        preload: function() {
	            for (var a in this.images) {
	                new Asset.image(this.images[a]);
	            }
	        }
	    };
         leopard.mask = (function() {
	        var a = null;
	        return {
	            show: function() {
	                if (!a) {
	                    a = new Mask(document.body, {
	                        style: {
	                            "background-color": "#333",
	                            opacity: 0.6
	                        }
       ...
       leopard.spinner = (function() {
	        ...
	        var j = null;
	        var m = {
	            wait: leopard.assets.images.working,
	            error: leopard.assets.images.error,
	            success: leopard.assets.images.success,
	            alert: leopard.assets.images.alert
	        };
	        var a = null;
	        var d = null;
	        var g = {};
	        function l() {
	            ...
	            new Hash({
	                ...
	                g[o] = new Element("input", {
	                    type: "button",
	                    value: p.label,
	                    "class": p.className,
	                    events: {
	                        click: function() {
	                            if (this.options.hideOnEvent) {
	                                this.hide();
	                            }
	                            this.fireEvent(o)
	                        }.bind(this)
	            ...
	            j = new leopard.Shadow({
	                mode: "frame",
	                offset: 5
	            });
	            f.addEvents({
	                display: j.show.bind(j, n),
	                close: j.hide.bind(j)
	            });
	        }
	        function i(n) {
	            ...
	            c.set("html", this.options.msg);
	            k.hide();
	            h.src = m[this.options.mode];
	            if (this.options.buttons.length > 0) {
	                e.getChildren().each(function(o) {
	                    o.dispose();
	                });
	                this.options.buttons.reverse().each(function(o) {
	                    if (g[o]) {
	                        e.adopt(g[o]);
	                    }
	                });
	            ...
	            if (this.options.position) {
	                f.position(this.options.position);
	            }
	        ...
	        function b() {
	            ...
	            if (this.options.hideAfter) {
	                d = function() {
	                    d = $clear(d);
	                    this.hide();
	                }.delay(this.options.hideAfter, this);
	            }
	        }
	        return {
	            show: function(n) {
	  ...
	  $extend(leopard.spinner, new Events());
	    leopard.dialogs = {
	        dismiss: function(b, c) {
	            var a = $(b).getParent(".dismiss");
	            if (a) {
	        ...
	        show_contact_dialog: function(a, e, c) {
	           	leopard.mask.show();
	           	leopard.spinner.wait("正在加载，请稍候...");
	            var b = new StickyWin.Ajax({
	                position: {
	                    x: "center",
	                    y: "top"
	                },
	            ...
	            b.update("/ajax/people/contact/?" + d.toQueryString())
	        },
	        show_album_dialog: function(a) {
	           	leopard.mask.show();
	           	leopard.spinner.wait("正在加载，请稍候...");
	            var b = new StickyWin.Ajax({
	                position: {
	            ...
	            if (a.callback) {
	                c.set("callback", a.callback);
	            }
	            b.update("/profile/create.html?" + c.toQueryString());
	        }
	    }
	    window.addEvent("domready", 
	    function() {
	    	    //leopard.assets.preload()
	    });
	})();
     
	function $decorate(d, a) {
	    d = $(d);
	    function b(c) {
	        return new Element("div").addClass(c)
	    }
	    var e = b(a + "-cc");
	    d.getChildren().each(function(c) {
	        c.dispose().inject(e)
	    });
	    b(a + "-t").adopt(b(a + "-tl")).adopt(b(a + "-tc")).adopt(b(a + "-tr")).inject(d);
	    b(a + "-c").adopt(b(a + "-cr").adopt(e)).inject(d);
	    b(a + "-b").adopt(b(a + "-bl")).adopt(b(a + "-bc")).adopt(b(a + "-br")).inject(d);
	    return d
	}
	leopard.Dropdown = new Class({
	    Implements: [Events, Options],
	    options: {
	        popup: true,
	        shadow: true,
	    ...
	    initialize: function(b, a) {
	        ...
	        if (this.options.isCurrent) {
	            this.menu.addClass("current");
	        }
	        this.showing = false;
	    },
	    initPopup: function() {
	        this.popup = $("dropdown_" + this.id);
	        ...
	        if (this.options.shadow) {
	            this.shadow = new leopard.Shadow({
	                ...
	                offset: this.options.shadowOffset
	            });
	            this.addEvents({
	                show: this.shadow.show.bind(this.shadow, this.popup),
	                hide: this.shadow.hide.bind(this.shadow)
	            });
	        }
	        this.popup.hide();
	        this.popup.dispose().injectInside(document.body)
	    },
	    mouseOver: function() {
	    ...
	    show: function() {
	        ...
	        if (!this.pos) {
	            var a = this.menu.getCoordinates();
	            this.pos = {
	        ...
	        if (Browser.Engine.trident4) {
	            if (!this.shim) {
	                this.shim = new IframeShim(this.popup);
	                this.addEvents({
	                    show: this.shim.show.bind(this.shim),
	                    hide: this.shim.hide.bind(this.shim)
	                });
	    ...
	    mouseUp: function(a) {
	        a = new Event(a);
	        if (this.showing) {
	            if (!this.popup.mouseOver(a.page) && !this.menu.mouseOver(a.page)) {
	                this.hide();
	                this.mouseOut();
	            }
	        }
	...
	var dropmenu = (function() {
	    ...
	    return {
	        init: function(d, e) {
	            d.each(function(f) {
	                f.isCurrent = (e == f.name);
	                var g = new leopard.Dropdown("menu_" + f.name, $extend(f, a));
	                g.addEvent("onMouseOver", b);
	                c.push(g);
	            })
	        }
	    }
